...
use crate::extract::{
    findings_in, non_recursive_checks, run_checks, scan_dict_keys, stamp_container, CheckOutcome,
    DocContext, Recurse, VectorCheck,
};
use crate::pdf;
...
impl VectorCheck for Attachments {
    ...
    fn run(&self, ctx: &DocContext, query: &Query) -> CheckOutcome {
        ...
        match ctx.recurse {
            Some(rec) => recurse_embedded(ctx, rec, &embedded_pdfs, query, &mut findings),
...
/// so cyclic or duplicated attachments are scanned once.
fn recurse_embedded(
    ctx: &DocContext,
    rec: Recurse,
    ...
    for (name, bytes) in embedded_pdfs {
        ...
        let lopdf_doc = Document::load_mem(bytes).ok();
        let pdfium_doc = ctx
            .pdfium_lib
            .and_then(|p| p.load_pdf_from_byte_vec(bytes.clone(), None).ok());
        let mut sub_ctx = DocContext::new(bytes, lopdf_doc.as_ref(), pdfium_doc.as_ref());
        ...
        stamp_container(&mut sub.findings, &format!("attachment:{name}"));
...
mod tests {
    ...
    fn pdf_bytes_with_xmp_secret(secret: &str) -> Vec<u8> {
        let mut doc = Document::with_version("1.5");
    ...
    fn host_with_attachment(name: &str, payload: &[u8], duplicate: bool) -> Document {
        let mut doc = Document::with_version("1.5");
