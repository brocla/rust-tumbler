...
/// Tumbler's redacted output of each neutralized attack (§8 closure). Every
/// name here has a fixture in `pentest-redacted/`, generated by Tumbler's
/// `dump_redacted_pentest_corpus` test. `corrupted-xref` is absent: Tumbler
/// refuses that file, so there is no redacted output to check.
const REDACTED_CLEAN: &[&str] = &[
    "all-vectors-combined",
    "annotation-appearance",
    "embedded-and-document-vectors",
    "form-xobject",
    "hidden-text-black-box",
    "incremental-update-cover",
    "invisible-render-mode",
    "masked-image-cover",
    "offpage-text",
    "optional-content-hidden",
...
#[test]
fn tumbler_redacted_output_has_no_findings() {
    // The headline §8 gate: run the checker on Tumbler's *redacted* output of
    // every attack and require 0 findings — Tumbler removes the secret, and
    // Subtext, checking a broader vector inventory than Tumbler's own
    // verifier, agrees it is gone. A finding here is a real gap between the two
    // (or a Subtext false positive), not a test to force green.
    let query = zanzibar_query();
    let dir = manifest_relative(&[
        "..", "..", "src-tauri", "tests", "fixtures", "redaction", "pentest-redacted",
    ]);
    for name in REDACTED_CLEAN {
        ...
        assert!(
            report.findings.is_empty(),
            "[{name}] Tumbler redacted this file, but Subtext still found the secret: {:#?}",
            report.findings
        );
        assert_ne!(
            report.risk_tone,
            RiskTone::Leak,
            "[{name}] redacted output must not read as a leak"
        );
    }
}
